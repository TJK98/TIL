...
* **반복적인 리소스 관리 코드**: 순수 JDBC로 Repository를 구현할 때, 모든 메서드마다 `Connection`, `PreparedStatement`를 열고 닫는 `try-catch-finally`
  코드가 반복되어 비효율적이고 실수할 여지가 많았다.

* **RowMapper의 람다식 형태**: `template.query(sql, (rs, rowNum) -> new Book(rs))` 코드에서 람다식이 `RowMapper` 인터페이스의 구현체라는 것을 이해하는
  데 시간이 걸렸다. `while(rs.next())` 루프와 객체 생성 코드를 한 줄의 람다로 표현하는 방식이 처음에는 낯설었다.

* **`query`와 `queryForObject`의 차이**: `queryForObject()`는 결과가 정확히 하나일 것이라고 예상될 때 사용하며, 결과가 없으면
  `EmptyResultDataAccessException` 예외가 발생한다는 점을 인지해야 했다.

* **`DataSource`의 자동 설정**: `new`로 생성한 적 없는 `DataSource` 객체가 Repository에 주입되는 원리가 궁금했다. `application.yml` 설정만으로 스프링 부트가
  `DataSource` 빈을 자동으로 생성하고 컨테이너에 등록해준다는 것을 알게 되었다.
...
* `JdbcTemplate`을 사용하면 개발자는 DB 커넥션, PreparedStatement 생성, 리소스 정리 등 반복적이고 오류가 발생하기 쉬운 작업을 프레-임워크에 위임할 수 있다. 이를 통해 핵심적인
  SQL 실행 및 결과 처리 로직에만 집중할 수 있다.
